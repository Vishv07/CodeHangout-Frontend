...
const HangoutArea = (props) => {
  ...
  useEffect(() => {
    ...
    const url = `${process.env.REACT_APP_CODEHANGOUT_BACKEND_API}/rooms`;
    console.log(url);
    ...
    fetch(url)
      ...
      .then((rooms) => {
        console.log(rooms);
        for (let i = 0; i < rooms.length; i++) roomsList.push(rooms[i]);
      });
    ...
    // this will send server the roomId in which the props.socket needs to be joined
    ...
    socket.on("addusers", (data) => {
      ...
      let msg = data.users[data.id] + " , Welcome to CodeHangout🤟";
    ...
    socket.on("userjoined", (users) => {
      ...
      let msg = newUser[id] + " , Welcome to CodeHangout🤟";
      props.setSnackBar(msg, "success");
        ...
        <Fragment>
          ...
          >
            <Grid container spacing={5}>
              ...
              <Grid item xs={12} sm={12} md={props.isFocusMode ? 12 : 6}>
                <SketchPad socket={socket} roomId={roomId} />
              </Grid>
            </Grid>
          </div>
        </Fragment>
...
export default connect(mapStateToProps, mapDispatchToProps)(HangoutArea);
